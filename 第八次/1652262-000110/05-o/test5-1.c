...
{  
    ...
    {  
        ...
        {
            strncpy(star_memory->buffer_share2, buffer_out, 2048);  
            printf("发送内容为:%s\n",buffer_out);      
            star_memory->write_flag2 = 1;  
        }
        while(star_memory->write_flag1)
        {
            printf("读取内容为:%s\n", star_memory->buffer_share1);  
            star_memory->write_flag1 = 0;//循环的从共享内存中读数据，直到读到“end”为止
        }
